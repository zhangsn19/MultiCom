...
Do not commit raw platform data or post text unless your license and sharing permissions allow it.
...
| `model` | `gpt-5.4-nano` | `OPENAI_MODEL` | Model used by every persona agent. |
...
| `inner_folds` | `4` | `MULTICOM_INNER_FOLDS` | Inner folds for tuning aggregation classifiers inside each outer fold. |
...
The OOF aggregator converts the 16 agent outputs into note-level vote, confidence, reason, failure-mode, disagreement, and metadata features. It then trains nested out-of-fold logistic calibration models and combines OOF predictors with a final hard ensemble plus a conservative promotion rule for selected `NEEDS_MORE_RATINGS` cases.
...
The OOF aggregator is leakage-safe: held-out predictions are generated by models that did not train on the held-out notes. The final `multicom_final` prediction combines multiple OOF views and applies a conservative promotion rule for selected `NEEDS_MORE_RATINGS` predictions.
...
The `.gitignore` excludes data, artifacts, model outputs, virtual environments, and local secrets. Keep raw data and generated predictions outside version control unless you intentionally publish them under an appropriate license.
